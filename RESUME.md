...
- **Ch6 — Te-form** (`ja-2-ch4`, dir `book2ch4`): 5 lessons ✅ NEW — the て-form, paced carefully. L1 Group 2 (る→て) + irregulars (する→して, くる→きて) + 〜てください (introduces plain/dictionary form). L2 Group 1 (godan) rules う・つ・る→って / む・ぶ・ぬ→んで / く→いて / ぐ→いで / す→して + いく→いって (kanji 聞 話). L3 〜ています ongoing actions + states すんでいます/しっています (kanji 読 書). L4 sequencing 〜て、〜 + 〜てから. L5 capstone (matching verb→て-form, mixed practice, Saturday microstory). Kanji **聞 話 読 書** (all verbs that drill the rules; real KanjiVG strokes). Placement: 2 questions + chapterOrder for ja-2-ch4.

**NEXT UP 👉 Book 2 Ch7 — Past tense** (`ja-2-ch5`, displayed Ch7 now): ました／でした, past verbs, past です (adjective past already covered in Ch4). Then Ch8 first-80 kanji, Ch9 reading. Consider a parallel Book 1 review checkpoint (user wants a summary after every book). Authoring recipe per chapter:
